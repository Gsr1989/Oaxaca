...
# ===================== ZONA HORARIA =====================
TZ_OAXACA = ZoneInfo("America/Mexico_City")

def now_oaxaca() -> datetime:
    return datetime.now(TZ_OAXACA)

def today_oaxaca() -> date:
    return now_oaxaca().date()
...
def parse_date_any(value) -> date:
    ...
    if isinstance(value, datetime):
        if value.tzinfo is None:
            value = value.replace(tzinfo=TZ_OAXACA)
        else:
            value = value.astimezone(TZ_OAXACA)
        return value.date()
    s = str(value).strip()
    ...
    if dt.tzinfo is None:
        dt = dt.replace(tzinfo=TZ_OAXACA)
    else:
        dt = dt.astimezone(TZ_OAXACA)
    return dt.date()

# ===================== FLASK CONFIG =====================
app = Flask(__name__)
app.secret_key = 'oaxaca_segura_123456'
...
PLANTILLA_PRINCIPAL = "oaxaca_plantilla_imagen.pdf"
PLANTILLA_SECUNDARIA = "oaxacaverga.pdf"
URL_CONSULTA_BASE = "https://oaxaca-gob-semovi.onrender.com"
ENTIDAD = "oaxaca"

os.makedirs(OUTPUT_DIR, exist_ok=True)

# ===================== FOLIOS OAXACA: 1 + CONSECUTIVO =====================
PREFIJO_OAXACA = "1"

def generar_folio_automatico_oaxaca():
    """
    Genera folio con prefijo 1 + consecutivo
    Formato: 1670, 1671, 1672... infinito
    Intenta 10,000,000 de veces hasta encontrar uno disponible
    """
    logger.info("[FOLIO] Iniciando generación automática OAXACA")
    
    # 1. TRAER TODOS LOS FOLIOS DE OAXACA
    ...
    # 2. FILTRAR SOLO LOS QUE SON DE OAXACA (empiezan con 1)
    consecutivos = []
    for f in todos:
        folio_str = str(f['folio'])
        if folio_str.startswith(PREFIJO_OAXACA) and len(folio_str) > 1:
            try:
    ...
    if not consecutivos:
        siguiente = 670  # Empezar en 1670
        logger.info(f"[FOLIO] Sin folios, empezando en {PREFIJO_OAXACA}{siguiente}")
    else:
        ultimo = max(consecutivos)
        siguiente = ultimo + 1
        logger.info(f"[FOLIO] Último: {PREFIJO_OAXACA}{ultimo}, siguiente: {PREFIJO_OAXACA}{siguiente}")
    
    # 4. BUSCAR HASTA 10,000,000 DE VECES
    for intento in range(10000000):
        folio_candidato = f"{PREFIJO_OAXACA}{siguiente + intento}"
...
def guardar_folio_con_reintento(datos, username):
    ...
    if not datos.get("folio"):
        try:
            datos["folio"] = generar_folio_automatico_oaxaca()
    ...
    # Extraer el número después de "1"
    try:
        num_inicial = int(folio_base[1:])
    except:
        num_inicial = 670
    
    for intento in range(max_intentos):
        folio_actual = f"{PREFIJO_OAXACA}{num_inicial + intento}"
...
def generar_pdf_unificado_oaxaca(datos: dict) -> str:
    ...
    if fecha_exp_dt.tzinfo is None:
        fecha_exp_dt = fecha_exp_dt.replace(tzinfo=TZ_OAXACA)
    else:
        fecha_exp_dt = fecha_exp_dt.astimezone(TZ_OAXACA)

    if isinstance(fecha_ven_dt, str):
        fecha_ven_str = fecha_ven_dt
    else:
        if fecha_ven_dt.tzinfo is None:
            fecha_ven_dt = fecha_ven_dt.replace(tzinfo=TZ_OAXACA)
        else:
            fecha_ven_dt = fecha_ven_dt.astimezone(TZ_OAXACA)
    ...
    try:
        ...
        pg1.insert_text(coords_oaxaca["folio"][:2], fol, 
        ...
        pg1.insert_text(coords_oaxaca["marca"][:2], datos["marca"], 
                        fontsize=coords_oaxaca["marca"][2], 
                        color=coords_oaxaca["marca"][3])
        
        pg1.insert_text(coords_oaxaca["serie"][:2], datos["numero_serie"], 
                        fontsize=coords_oaxaca["serie"][2], 
                        color=coords_oaxaca["serie"][3])
        
        pg1.insert_text(coords_oaxaca["linea"][:2], datos["linea"], 
                        fontsize=coords_oaxaca["linea"][2], 
                        color=coords_oaxaca["linea"][3])
        
        pg1.insert_text(coords_oaxaca["motor"][:2], datos["numero_motor"], 
                        fontsize=coords_oaxaca["motor"][2], 
                        color=coords_oaxaca["motor"][3])
        
        pg1.insert_text(coords_oaxaca["anio"][:2], str(datos["anio"]), 
                        fontsize=coords_oaxaca["anio"][2], 
                        color=coords_oaxaca["anio"][3])
        
        pg1.insert_text(coords_oaxaca["color"][:2], datos.get("color", "N/A"), 
                        fontsize=coords_oaxaca["color"][2], 
                        color=coords_oaxaca["color"][3])
        ...
        # QR dinámico
        img_qr, _ = generar_qr_dinamico_oaxaca(fol)
...
def login():
    if request.method == 'POST':
        ...
        resp = supabase.table("usuarios_oaxaca")\
            .select("*")\
...
def crear_usuario():
    ...
    if request.method == 'POST':
        ...
        existe = supabase.table("usuarios_oaxaca")\
            .select("id")\
            .eq("username", username)\
            .limit(1)\
            .execute()
        ...
        if existe.data:
            ...
            supabase.table("usuarios_oaxaca").insert({
                "username": username,
                "password": password,
...
def registro_usuario():
    ...
    user_data = supabase.table("usuarios_oaxaca")\
        .select("*")\
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            fecha_inicio = fecha_inicio.replace(tzinfo=TZ_OAXACA)
        except:
        ...
        generar_pdf_unificado_oaxaca(datos)

        supabase.table("usuarios_oaxaca")\
...
def mis_permisos():
    ...
    usr_data = supabase.table("usuarios_oaxaca")\
...
def descargar_pdf(folio):
    ...
    return send_file(
        ruta_pdf,
        as_attachment=True,
        download_name=f"{folio}_oaxaca.pdf",
...
TABLAS_DISPONIBLES = {
    'folios_registrados': {
        ...
        'columnas': ['folio', 'marca', 'linea', 'anio', 'numero_serie', 'numero_motor', 
                     'color', 'nombre', 'fecha_expedicion', 'fecha_vencimiento', 'entidad', 'estado', 'creado_por']
    },
    'usuarios_oaxaca': {
...
if __name__ == '__main__':
    logger.info("🚀 SERVIDOR OAXACA INICIADO")
